...
- **기본 기능**
  ...
  - 기존 HTML 내보내기 기능을 활용하여 PDF 생성
  - PDF 파일 저장 위치 선택 가능
  - 임시 HTML 생성 후 PDF 변환하여 최종 PDF만 저장

- **PDF 생성 워크플로**
  1. 기존 HTML 내보내기 서비스를 사용하여 임시 HTML 생성
  2. 생성된 HTML과 이미지를 PDF로 변환
  3. 임시 파일들 정리 후 최종 PDF 파일만 유지
  4. 사용자에게 저장 완료 알림

- **PDF 변환 기술**
  - `weasyprint` 라이브러리 사용 (HTML to PDF 변환)
  - CSS 스타일링 완전 지원
  - 고품질 이미지 렌더링
  - 페이지 크기: A4 또는 사용자 정의

- **PDF 문서 구조**
  - HTML 내보내기와 동일한 구조를 PDF로 변환
  - **첫 페이지**: Living Collage 제목, 생성일, 콜라주 이미지
  - **후속 페이지**: 가구 목록 정보 (필요시 여러 페이지)

- **PDF 최적화**
  - 이미지 품질 최적화로 파일 크기 관리
  - 텍스트 검색 가능한 PDF 생성
  - 인쇄용 최적화 (300 DPI)

- **에러 처리**
  - weasyprint 설치 확인 및 안내 메시지
