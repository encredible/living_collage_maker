...
- **가구 정보 표시 내용**
  ...
  - 설명 (description)
  - 색상 (color)
  - 크기 정보 (width, depth, height) - cm 단위
  - 좌석 높이 (seat_height) - cm 단위, 해당하는 경우만
  - 위치 정보 (locations) - 리스트 형태로 표시
  - 스타일 정보 (styles) - 리스트 형태로 표시
  - 제품 링크 - "제품 바로가기" 하이퍼링크
...
- **HTML 스타일링**
  - 깔끔하고 읽기 쉬운 기본 레이아웃
  - 가구 정보는 테이블 또는 간단한 목록 형태로 구성
  - 기본 CSS 스타일 내장으로 외부 파일 의존성 없음
  - 이미지가 적당한 크기로 잘 보이도록 설정
